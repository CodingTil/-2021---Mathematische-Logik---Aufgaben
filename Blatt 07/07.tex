...
\begin{enumerate}[label=(\alph*)]
	...
	\item	Wir wissen unter anderem aus Hausaufgabe 05 Aufgabe 04, dass solche Strukturen ein Nullelement, das sogenannte leere Wort $\epsilon$ besitzen, welches wir mit einer einfachen Formel $\varphi$ bestimmen können. $\varphi(x)$ sei also genau dann erfüllt, wenn $x$ das leere Wort $\epsilon$ ist.\\
			...
			Angenommen es gäbe einen Automorphismus $\pi$, sodass irgendein $x \in \Sigma$ $pi(x) \not\in \Sigma$. Dann muss es ein Wort $w \in \Sigma^*$ mit $\vert w \vert > 1$, sodass $\pi(w) \in \Sigma$. Jetzt können wir $w$ durch $\circ$ in alle $w_i \in \Sigma, 1 \leq i \leq \vert w \vert = n$ aufteilen mit $w = w_1 \circ \dots \circ w_n$. Da nur $\pi(\epsilon) = \epsilon$, kann keines der $w_i$ auf $\epsilon$ abbilden. Somit muss $\vert \pi(w) \vert \geq n$ sein, was ein Widerspruch ist. Somit gilt für alle $y \in \Sigma$, dass $\pi(y) \in \Sigma$.\\
			Da jedes $v \in \Sigma^*, m = \vert v \vert \geq 1$ in $v_i \in \Sigma$ aufgeteilt werden kann und $\pi(v) = \pi(v_1) \circ \dots \circ \pi(v_m)$, muss $\vert v \vert = \vert \pi(v) \vert$.\\
			
			Somit kann man alle Automorphismen auf $\mathfrak{A}$ so beschreiben, die jedem Buchstaben einen anderen (oder gleichen) Buchstaben zuordnen.
\end{enumerate}
...
\section*{Aufgabe 6}
\begin{enumerate}[label=(\alph*)]
	\item	Es gibt den nicht-trivialen Automorphismus $\pi: x \mapsto x \cdot 2$. Angenommen es gäbe ein $\psi \in \operatorname{FO}(\tau)$, das $\mathbb{Q}_{\geq 1}$ elementar definiert.\\
			Dann können wir $1$ definieren als $1(x) \coloneqq \mathbb{Q}_{\geq 1}x \land \forall y (\mathbb{Q}_{\geq 1}y \impl (x < y \lor x = y))$.\\
			Dann ist aber $\pi$ kein Automorphismus mehr, denn $\psi(\pi(x)) \neq \psi(x)$ für ein $x \in \mathbb{Q}$, denn es existiert ja ein $x$ mit $1(x)$, also $x=1$, sodass $\psi(\pi(1)) = \psi(2) \neq \psi(1)$.
	\item	\begin{align*}
				\varphi(n,p) \coloneqq & (n < p \lor n=p) \land 1 < p \land (\forall y \forall z (y \cdot z = p \impl (y=1 \lor z=1))) \\ & \land \forall x ((n < x \lor x < p) \impl \neg (\forall y \forall z (y \cdot z = p \impl (y=1 \lor z=1))))
			\end{align*}
			Besteht aus $n \leq p$ und $p \in \mathbb{P}$ und alle Zahlen dazwischen sind keine Primzahlen.
	\item	In unserer Struktur gibt es den Automorphismus $\pi: x \mapsto x + (2 \cdot 4 \cdot 7)$. Damit verschieben wir intuitiv den gemeinsamen Nullpunkt.\\
			Somit kann kein $\psi \in \operatorname{FO}(\tau)$ geben, das $11\mathbb{Z}$ elementar definiert, weil für diese Formel nicht gilt: $\psi(\pi(x)) = \psi(x)$ für alle $x \in \mathbb{Z}$
