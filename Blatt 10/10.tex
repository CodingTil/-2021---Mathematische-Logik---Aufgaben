...
\begin{enumerate}[label=(\alph*)]
	\item	\begin{enumerate}[label=(\roman*)]
				...
				\item	\begin{enumerate}[label=(\arabic*)]
							\item	Sei $t$ ein beliebiger Term. Dann ist $t=t \in \operatorname{Th}(\A)$, denn $\A \models t=t$, denn $t=t$ ist eine Tautologie.
							\item	Seien $t,t'$ beliebige Terme. Angenommen $t=t',\psi(t) \in \operatorname{Th}(\A)$. Dann ist auch $\psi(t') \in \operatorname{Th}(\A)$, denn es gilt: $\{t = t', \psi(t)\} \models \psi(t')$ und Theorien sind unter $\models$ abgeschlossen.
						 \end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
	...
	\item	Gegenbeispiel: Die Klasse der Endlichen Linearen Ordnungen welche endlich axiomatisierbar ist durch $\phi = \forall x (\forall y (y < x \lor y = x) \lor \exists y (x < y \land \neg \exists z (x < z < y))) \land \exists x (\forall y (x < y \lor x = y) \land \forall x \forall y \forall z (x < y \land y < z \Rightarrow x < z) \land \forall x \forall y (x < y \lor y < x \lor x = y)$\\
			...
			Sei $\psi_n = \exists x_1 \exists x_2 \dots \exists x_n ( x_1 \not = x_2 \land x_1 \not = x_3 \land \dots \land x_1 \not = x_n \land x_2 \not = x_3 \land \dots \land x_{n-1} \not = x_n \land \forall y ((y = x_1 \lor y = x_2 \lor \dots \lor y = x_n) \land x_1 < x_2 \land x_2 < x_3 \land \dots \land x_{n-1} < x_n) \land \forall x \forall y \forall z (x < y \land y < z \Rightarrow x < z) \land \forall x \forall y (x < y \lor y < x \lor x = y)$

			$\psi_n$ axiomatisiert die Lineare Ordnung mit n Elementen. Also ist $\Psi = \{\psi_n \mid n \in \mathbb{N}\}$ ein unendliches Axiomensystem für die Endlichen Linearen Ordnungen.
